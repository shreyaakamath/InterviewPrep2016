...
public class BinarySearchTree {
	...
	void insert(Node n1 , int num){
		if(root == null) {root = new Node(num, null, null, null);sumExists=false;sameTree=true;isBST= true;return;}
		if(num<n1.key){
	...
	/*check if two BST are same by comparing corresponding nodes*/
	boolean isLeaf(Node n1){
		return(n1.left==null&&n1.right==null);
	}
	boolean sameTree(Node n1 , Node n2){
		if(n1!=null && n2!=null){
		if(!isLeaf(n1)){
			//n1 is not leaf but n2 is leaf
			if(isLeaf(n2)) sameTree=false;
			//both are not leaves
			else {
				if(n1.key != n2.key) sameTree = false;
			}
		} else {
			//n1 is leaf 
			if(!isLeaf(n2)) sameTree=false;
	...
	/*return min/max value of binary tree */
	int minMaxValueOfBinaryTree(Node n1, int maxSoFar , boolean calculateMax){
		if(!isLeaf(n1)){
			List<Integer> val = new ArrayList<Integer>();
			val.add(n1.key);
			val.add(n1.left.key);
			val.add(n1.right.key);
			if(calculateMax){
				if(Collections.max(val) > maxSoFar) maxSoFar=Collections.max(val);
			} else {
				if(Collections.min(val) < maxSoFar) maxSoFar=Collections.min(val);
			}
			
			minMaxValueOfBinaryTree(n1.left,maxSoFar,calculateMax);
			minMaxValueOfBinaryTree(n1.right,maxSoFar,calculateMax);
			
			}
			return maxSoFar;
	}
	
	/*use the above helper function to determine if binary tree given is bst*/
	boolean isBST(Node n1){if((minMaxValueOfBinaryTree(n1.left,n1.left.key,false) <= n1.key) && (minMaxValueOfBinaryTree(n1.right,n1.right.key,true) > n1.key)) return true; 
		return false;
	}
	...
	public static void main(String args[]){
		...
		for(int num:b) t2.insert(t2.root, num);
		t1.inorderWalk(t1.root);

		
		System.out.println("------");
		/*t1.root.right.key=0;
		System.out.println(t1.isBST(t1.root));
		System.out.println(t1.sameTree(t1.root, t2.root));
